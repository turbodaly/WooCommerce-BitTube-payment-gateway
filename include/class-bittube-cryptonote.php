...
require_once('class-bittube-base58.php');

class BitTube_Cryptonote
{
    protected $ed25519;
    public function __construct()
    {
        $this->ed25519 = new ed25519();
        $this->base58 = new BitTube_base58();
        $this->address_prefix = BITTUBE_GATEWAY_ADDRESS_PREFIX;
        $this->address_prefix_integrated = BITTUBE_GATEWAY_ADDRESS_PREFIX_INTEGRATED;
    }
    ...
    /*
     * Create a valid base58 encoded BitTube address from public keys
     *
     * @param string Public spend key
     * @param string Public view key
     *
     * @return string Base58 encoded BitTube address
     */
...
/*
 * Decode a base58 encoded BitTube address
 *
 * @param string A base58 encoded BitTube address
 *
 ...
 */
    ...
    public function integrated_addr_from_keys($public_spendkey, $public_viewkey, $payment_id)
    {
        ...
        $checksum = substr($this->keccak_256($data), 0, 16);
        $result = $this->base58->encode($data.$checksum);
        return $result;
    }

    /*
     * Generate a BitTube address from seed
     *
     * @param string Hex string to use as seed
     *
     * @return string A base58 encoded BitTube address
     */
