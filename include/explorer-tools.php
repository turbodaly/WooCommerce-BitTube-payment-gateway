...
class BitTube_Explorer_Tools {
    private $url;
    public function __construct($testnet = false) {
        $this->url = $testnet ? BITTUBE_GATEWAY_TESTNET_EXPLORER_URL : BITTUBE_GATEWAY_MAINNET_EXPLORER_URL;
    }

    private function call_api($endpoint) {
        $http     = new WP_Http();
        $response = $http->request($this->url . $endpoint);
    ...
    public function get_mempool_txs() {
