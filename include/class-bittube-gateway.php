<?php
/*
 * Main Gateway of BitTube using either a local daemon or the explorer
 * Authors: Serhack, cryptochangements, mosu-forge
 */

defined( 'ABSPATH' ) || exit;

require_once('class-bittube-cryptonote.php');
require_once ABSPATH . WPINC . '/class-http.php';

class BitTube_Gateway extends WC_Payment_Gateway
{
    private static $_id = 'bittube_gateway';
    private static $_title = 'BitTube Gateway';
    private static $_method_title = 'BitTube Gateway';
    private static $_method_description = 'BitTube Gateway Plug-in for WooCommerce.';
    ...
    private static $use_bittube_price = false;
    private static $use_bittube_price_decimals = BITTUBE_GATEWAY_ATOMIC_UNITS;

    private static $cryptonote;
    private static $bittube_wallet_rpc;
    private static $bittube_explorer_tools;
    private static $log;
    ...
    {
		return apply_filters('woocommerce_gateway_icon', '<img src="'.BITTUBE_GATEWAY_PLUGIN_URL.'assets/images/bittube-icon.png"/>');
    ...
    {
        $this->id = self::$_id;
        $this->method_title = __(self::$_method_title, 'bittube_gateway');
        $this->method_description = __(self::$_method_description, 'bittube_gateway');
        ...
        self::$address = $this->settings['bittube_address'];
        ...
        self::$use_bittube_price = $this->settings['use_bittube_price'] == 'yes';
        self::$use_bittube_price_decimals = $this->settings['use_bittube_price_decimals'];

        $explorer_url = self::$testnet ? BITTUBE_GATEWAY_TESTNET_EXPLORER_URL : BITTUBE_GATEWAY_MAINNET_EXPLORER_URL;
        defined('BITTUBE_GATEWAY_EXPLORER_URL') || define('BITTUBE_GATEWAY_EXPLORER_URL', $explorer_url);

        if($add_action)
            add_action('woocommerce_update_options_payment_gateways_'.$this->id, array($this, 'process_admin_options'));

        // Initialize helper classes
        self::$cryptonote = new BitTube_Cryptonote();
        if(self::$confirm_type == 'bittube-wallet-rpc') {
            require_once('class-bittube-wallet-rpc.php');
            self::$bittube_wallet_rpc = new BitTube_Wallet_Rpc(self::$host, self::$port);
        } else {
            require_once('class-bittube-explorer-tools.php');
            self::$bittube_explorer_tools = new BitTube_Explorer_Tools(self::$testnet);
        }
    ...
    {
        $this->form_fields = include 'admin/bittube-gateway-admin-settings.php';
    }

    public function validate_bittube_address_field($key,$address)
    {
        if($this->settings['confirm_type'] == 'viewkey') {
            if (strlen($address) == 97 )
                if(self::$cryptonote->verify_checksum($address))
                    return $address;
            self::$_errors[] = 'BitTube address is invalid';
        }
        return $address;
    }
    ...
    public function admin_options()
    {
        ...
        if($confirm_type === 'bittube-wallet-rpc')
        ...
        include BITTUBE_GATEWAY_PLUGIN_DIR . '/templates/bittube-gateway/admin/settings-page.php';
    }
    ...
    public static function admin_balance_info()
    {
        ...
        $wallet_amount = self::$bittube_wallet_rpc->getbalance();
        $height = self::$bittube_wallet_rpc->getheight();
        if (!isset($wallet_amount)) {
            self::$_errors[] = 'Cannot connect to bittube-wallet-rpc';
            self::$log->add('BitTube_Payments', '[ERROR] Cannot connect to bittube-wallet-rpc');
            return array(
                'height' => 'Not Available',
                'balance' => 'Not Available',
                'unlocked_balance' => 'Not Available',
            ...
            return array(
                'height' => $height,
                'balance' => self::format_bittube($wallet_amount['balance']).' BitTube',
                'unlocked_balance' => self::format_bittube($wallet_amount['unlocked_balance']).' BitTube'
            );
        }
    }
    ...
    public function process_payment($order_id)
    {
        ...
        $bittube_amount = 1e8 * $fiat_amount / $rate;

        if(self::$discount)
            $bittube_amount = $bittube_amount - $bittube_amount * self::$discount / 100;

        $bittube_amount = intval($bittube_amount * BITTUBE_GATEWAY_ATOMIC_UNITS_POW);

        $query = $wpdb->prepare("INSERT INTO $table_name (order_id, payment_id, currency, rate, amount) VALUES (%d, %s, %s, %d, %d)", array($order_id, $payment_id, $currency, $rate, $bittube_amount));
        $wpdb->query($query);

        $order->update_status('on-hold', __('Awaiting offline payment', 'bittube_gateway'));
    ...
    public static function do_update_event()
    {
        ...
        $api_link = 'https://min-api.cryptocompare.com/data/price?fsym=TUBE&tsyms='.$currencies.'&extraParams=bittube_woocommerce';
		$curl = curl_init();
        ...
        if(self::$confirm_type == 'bittube-wallet-rpc')
            ...
            $height = self::$bittube_explorer_tools->getheight();
        set_transient('bittube_gateway_network_height', $height);

        // Get pending payments
        $table_name_1 = $wpdb->prefix.'bittube_gateway_quotes';
        $table_name_2 = $wpdb->prefix.'bittube_gateway_quotes_txids';
        ...
        foreach($pending_payments as $pending) {
            ...
            $amount_bittube = $quote->amount_total;
            ...
            $paid = $amount_paid > $amount_bittube - BITTUBE_GATEWAY_ATOMIC_UNIT_THRESHOLD;
            ...
            if($paid && $confirmed) {
                self::$log->add('BitTube_Payments', "[SUCCESS] Payment has been confirmed for order id $order_id and payment id $payment_id");
                ...
                if(self::is_virtual_in_cart($order_id) == true){
                    $order->update_status('completed', __('Payment has been received.', 'bittube_gateway'));
                } else {
                    $order->update_status('processing', __('Payment has been received.', 'bittube_gateway'));
                }

            } else if($paid) {
                self::$log->add('BitTube_Payments', "[SUCCESS] Payment has been received for order id $order_id and payment id $payment_id");
                ...
                if($order_age_seconds > self::$valid_time) {
                    self::$log->add('BitTube_Payments', "[FAILED] Payment has expired for order id $order_id and payment id $payment_id");
                    $query = $wpdb->prepare("UPDATE $table_name_1 SET pending=0 WHERE payment_id=%s", array($payment_id));
                    $wpdb->query($query);

                    unset(self::$payment_details[$order_id]);

                    $order->update_status('cancelled', __('Payment has expired.', 'bittube_gateway'));
                }
            }
        }
    }
    ...
    {
        $txs = array();
        $payments = self::$bittube_wallet_rpc->get_all_payments($payment_id);
    ...
    {
        $txs = array();
        $outputs = self::$bittube_explorer_tools->get_outputs(self::$address, self::$viewkey);
        ...
        if (count($details)) {
            ...
            foreach($details as $tx) {
                ...
                $txs[] = array(
                    ...
                    'amount_formatted' => self::format_bittube($tx->amount_paid)
                );
            ...
            if(count($heights) && !in_array(0, $heights)) {
                $height = get_transient('bittube_gateway_network_height');
                $highest_block = max($heights);
            ...
            $time_to_confirm = self::format_seconds_to_time($blocks_to_confirm * BITTUBE_GATEWAY_DIFFICULTY_TARGET);
            ...
            if(self::$confirm_type == 'bittube-wallet-rpc') {
                $array_integrated_address = self::$bittube_wallet_rpc->make_integrated_address($payment_id);
                if (isset($array_integrated_address['integrated_address'])) {
                    $integrated_addr = $array_integrated_address['integrated_address'];
                } else {
                    self::$log->add('BitTube_Gateway', '[ERROR] Unable get integrated address');
                ...
                if ($address) {
                    ...
                    self::$log->add('BitTube_Gateway', '[ERROR] Merchant has not set BitTube address');
                    return '[ERROR] Merchant has not set BitTube address';
                }
            }
            ...
            $qrcode_uri = "bittube:".$address."&tx_payment_id=".$payment_id;
            ...
            $payment_details = array(
                ...
                'rate' => $details[0]->rate,
                'rate_formatted' => sprintf('%.3f', $details[0]->rate / 1e8),
                'currency' => $details[0]->currency,
                'amount_total' => $amount_total,
                'amount_paid' => $amount_paid,
                'amount_due' => sprintf('%.6f', $amount_due),
                'amount_total_formatted' =>  sprintf('%.8f',$amount_total / 1e8),
                'amount_paid_formatted' => sprintf('%.8f',$amount_paid / 1e8),
                'amount_due_formatted' => sprintf('%.8f', $amount_due / 1e8),
                'status' => $status,
    ...
    public static function get_payment_details_ajax() {
        ...
        if($order->get_payment_method() != self::$_id)
            self::ajax_output(array('error' => '[ERROR] Order not paid for with BitTube'));
    ...
    public static function admin_order_page($post)
    {
        ...
        if(!is_array($details)) {
            $error = $details;
            include BITTUBE_GATEWAY_PLUGIN_DIR . '/templates/bittube-gateway/admin/order-history-error-page.php';
            return;
        }
        include BITTUBE_GATEWAY_PLUGIN_DIR . '/templates/bittube-gateway/admin/order-history-page.php';
    }
    ...
    public static function customer_order_page($order)
    {
        ...
        if(!is_array($details)) {
            $error = $details;
            include BITTUBE_GATEWAY_PLUGIN_DIR . '/templates/bittube-gateway/customer/order-error-page.php';
            return;
        }
        $show_qr = self::$show_qr;
        $details_json = json_encode($details);
        $ajax_url = WC_AJAX::get_endpoint('bittube_gateway_payment_details');
        include BITTUBE_GATEWAY_PLUGIN_DIR . '/templates/bittube-gateway/customer/order-page.php';
    }
    ...
    public static function customer_order_email($order)
    {
        ...
        $details = self::get_payment_details($order_id);
        if(!is_array($details)) {
            include BITTUBE_GATEWAY_PLUGIN_DIR . '/templates/bittube-gateway/customer/order-email-error-block.php';
            return;
        }
        include BITTUBE_GATEWAY_PLUGIN_DIR . '/templates/bittube-gateway/customer/order-email-block.php';
    }
    ...
    public static function use_bittube_price()
    {
        return self::$use_bittube_price;
    }
    ...
    {
        $rate = self::get_live_rate($currency);
        $bittube_amount = intval(BITTUBE_GATEWAY_ATOMIC_UNITS_POW * 1e8 * $price / $rate) / BITTUBE_GATEWAY_ATOMIC_UNITS_POW;
        $bittube_amount_formatted = sprintf('%.'.self::$use_bittube_price_decimals.'f', $bittube_amount);

        return <<<HTML
            <span class="woocommerce-Price-amount amount" data-price="$price" data-currency="$currency"
        data-rate="$rate" data-rate-type="live">
            $bittube_amount_formatted
            <span class="woocommerce-Price-currencySymbol">TUBE</span>
        </span>
...
HTML;
    ...
    public static function convert_wc_price_order($price_html, $order)
    {
        ...
        $bittube_amount = intval(BITTUBE_GATEWAY_ATOMIC_UNITS_POW * 1e8 * $price / $rate) / BITTUBE_GATEWAY_ATOMIC_UNITS_POW;
        $bittube_amount_formatted = sprintf('%.'.BITTUBE_GATEWAY_ATOMIC_UNITS.'f', $bittube_amount);
...
HTML;
    ...
    public static function format_bittube($atomic_units) {
        return sprintf(BITTUBE_GATEWAY_ATOMIC_UNITS_SPRINTF, $atomic_units / BITTUBE_GATEWAY_ATOMIC_UNITS_POW);
    }
